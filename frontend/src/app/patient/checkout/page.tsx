...
import { BadgeCheck, ClipboardList, ChevronDown, ShoppingCart, Lock, ShieldCheck } from 'lucide-react';

export default function CheckoutPage() {
  return (
    <div className="font-body-md text-on-surface antialiased min-h-screen flex flex-col bg-[#f8fafc]">
      {/* TopNavBar */}
      <header className="bg-surface dark:bg-inverse-surface border-b border-outline-variant dark:border-outline sticky top-0 z-50">
        <div className="flex justify-between items-center w-full px-margin py-base max-w-max-width mx-auto">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <a className="text-on-surface-variant dark:text-on-secondary-fixed-variant text-label-md font-label-md hover:text-primary dark:hover:text-primary-fixed transition-colors" href="#">Consult</a>
            <a className="text-on-surface-variant dark:text-on-secondary-fixed-variant text-label-md font-label-md hover:text-primary dark:hover:text-primary-fixed transition-colors" href="#">AI Triage</a>
            <a className="text-primary dark:text-primary-fixed border-b-2 border-primary dark:border-primary-fixed pb-1 text-label-md font-label-md opacity-80 transition-opacity" href="#">Order</a>
            <a className="text-on-surface-variant dark:text-on-secondary-fixed-variant text-label-md font-label-md hover:text-primary dark:hover:text-primary-fixed transition-colors" href="#">Library</a>
          </nav>
          <div className="flex items-center gap-4">
            <button className="text-on-surface-variant hover:text-primary text-label-md font-label-md hidden md:block">Language</button>
            <button className="bg-primary text-on-primary px-4 py-2 rounded-lg text-label-md font-label-md hover:bg-primary-container hover:text-on-primary-container transition-colors shadow-sm">Login</button>
          </div>
        </div>
      </header>
        ...
        <div className="col-span-1 md:col-span-7 flex flex-col gap-md">
          ...
          <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-md flex items-center gap-md">
            <img 
              ...
              alt="A professional headshot of a mature female doctor wearing a white clinical coat and a stethoscope" 
              src="https://lh3.googleusercontent.com/aida-public/AB6AXuCU_E0VPiOo60ySn9pUYGRoQkJfbB7XM0QOLOEO86wqwhEcbGnjgPNM2UN1tc8e8ZlsrEEQ2xojRbsYrch2xK-GFxxLJocIJm-5XBlk6lHHgeWq9jLS0n5IetuiNlM5uczEBu3bb32MF6Q043GcH-G-A24Ao_NKCDEwQEJQl5SwGz0BCmxhzoTvUcHtq6BwwRqwQsJMb-5MDvyEbMPgW9jMy0WObTkedQhBwyGWYQuGj4uSIzSjvdu4CMBvzNGRoSjU7ijIi5onwjiL" 
            />
            <div>
      ...
      <footer className="bg-surface-container-highest dark:bg-inverse-surface mt-auto">
        <div className="w-full py-lg px-margin flex flex-col md:flex-row justify-between items-center max-w-max-width mx-auto">
          ...
          <div className="flex gap-6">
            <a className="text-on-surface-variant dark:text-on-secondary-fixed-variant text-label-sm font-label-sm hover:text-primary dark:hover:text-primary-fixed transition-colors" href="#">Privacy Policy</a>
            <a className="text-on-surface-variant dark:text-on-secondary-fixed-variant text-label-sm font-label-sm hover:text-primary dark:hover:text-primary-fixed transition-colors" href="#">Terms of Service</a>
            <a className="text-on-surface-variant dark:text-on-secondary-fixed-variant text-label-sm font-label-sm hover:text-primary dark:hover:text-primary-fixed transition-colors" href="#">Healthcare Ethics</a>
            <a className="text-on-surface-variant dark:text-on-secondary-fixed-variant text-label-sm font-label-sm hover:text-primary dark:hover:text-primary-fixed transition-colors" href="#">Contact Us</a>
          </div>
        </div>
      </footer>
    </div>
